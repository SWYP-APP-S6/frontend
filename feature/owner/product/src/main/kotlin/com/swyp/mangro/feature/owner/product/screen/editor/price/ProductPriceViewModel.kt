...
class ProductPriceViewModel @Inject constructor(private val savedStateHandle: SavedStateHandle) : ViewModel() {
    ...
    fun initialize(draft: ProductDraftModel) {
        ...
        update(ProductPriceState(isLoading = false))
    ...
    fun handleAction(action: ProductPriceAction) {
        ...
        when (action) {
            ...
            ProductPriceAction.NextClicked -> if (state.canContinue) _event.trySend(ProductPriceEvent.Next(checkNotNull(savedStateHandle.get<ProductDraftModel>("draft")).copy(originalPrice = state.originalPrice.toInt(), salePrice = state.salePrice.toInt(), quantity = state.quantity)))
