 ...
 *
 * 진행률이 [thresholdValue]를 초과하면 그라데이션과 마커가 기준값 이후 색상으로 전환된다.
 *
 * 호출부도 [progress] 람다 안에서 State를 읽어야 한다.
 *
...
fun MangroGradientProgressBar(
    ...
    val isDanger by remember(progress, thresholdValue) {
        derivedStateOf { progress() > thresholdValue }
    }
    ...
    Canvas(
        ...
        val value = progress()
        val fraction = if (value.isNaN()) 0f else value.coerceIn(0f, 1f)
...
private fun MangroGradientProgressBarAnimationPreview() {
    val transition = rememberInfiniteTransition(label = "Progress threshold preview")
