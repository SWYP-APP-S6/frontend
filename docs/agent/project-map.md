...
- Flavor별 `MainScreen`: `app/src/consumer`, `app/src/owner`
  - Owner는 홈·점포 관리·찜 Navigation을 사용한다. 온보딩 모듈의 실제 주소 검색은 연결되어 있으며 등록 API와 최상위 이동은 미연결이다.
...
- Consumer `MainScreen`: `app/src/consumer`. `AppNavGraph`와 스플래시 시작 테마는 `app/src/main`에서 공유한다.
  - Consumer는 스플래시에서 로그인 화면으로 진입하고, Owner Debug와 Release는 홈에서 상품·찜 화면으로 이동한다.
...
- `:app`은 `:core:designsystem`, `:core:utils`에 의존하며, Owner에 한해 `:feature:owner:onboarding`, `:feature:owner:home`, `:feature:owner:product`에도 의존한다. Data 모듈은 아직 등록되지 않았다.
