...
class InputFilesTest(unittest.TestCase):
    ...
    def test_partial_and_older_specs_do_not_replace_full_0919_v2_merged_spec(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            for name in INPUT_FILES[1:]:
                (root / name).touch()
            (root / 'mangro-app-openapi-2026-09-17-v3.json').touch()
            (root / 'mangro-app-openapi-2026-09-19-v2.json').touch()
            (root / 'mangro-app-openapi-2026-09-19-merged.json').touch()
            with self.assertRaisesRegex(ValueError, '09-19-v2-merged.json'):
                check_inputs(root)
