...
class OwnerContractTest {
    ...
    fun stockUpdateUsesTotalStockAndCancelOverflow() = runTest {
        MockWebServer().use { server ->
            ...
            OwnerServices(NetworkClient.create(server.url("/").toString())).product
            ...
            assertEquals(NetworkClient.json.parseToJsonElement("""{"stockQty":12,"cancelOverflow":true}"""), NetworkClient.json.parseToJsonElement(request.body.readUtf8()))
        }
    }
    ...
    fun reconfirmPreservesFalseAndPreviewUsesSeparatePost() = runTest {
        MockWebServer().use { server ->
            val product = OwnerServices(NetworkClient.create(server.url("/").toString())).product
    ...
    fun photoUploadSendsMultipartFilePart() = runTest {
        MockWebServer().use { server ->
            ...
            val response = OwnerServices(NetworkClient.create(server.url("/").toString())).product.uploadProductPhoto(part)
    ...
    fun v2StockFieldsDecodeWithoutUsingDefaults() {
        val stock = NetworkClient.json.decodeFromString<ProductDetailResponse>("""{"stockQty":2,"shortfallQty":3,"activeHoldQty":5,"reconfirmPending":true,"stockEditable":false,"minAdjustableQty":2}""")
    ...
    fun unknownEnumDoesNotSilentlyBecomeDefault() {
        assertThrows(SerializationException::class.java) {
            NetworkClient.json.decodeFromString<RegisterProductRequest>("""{"category":"FUTURE"}""")
        }
    }
}
