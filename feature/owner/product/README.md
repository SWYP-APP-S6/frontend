...
- `ProductEditorNavHost`에서 사진·품목명 → 수량·가격 → 픽업시간·태그로 이동한다. 각 ViewModel이 자신의 기본값이 있는 State를 SavedStateHandle에 보존한다. 다음 단계에는 ProductDraftModel만 전달하고, 상단/시스템 뒤로가기는 이전 목적지로 돌아간다. 미리보기의 수정 액션은 입력을 유지한 채 첫 목적지로 돌아간다.
