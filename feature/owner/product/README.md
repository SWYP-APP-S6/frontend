...
- `GET /owner/products?filter=ALL&page=0&size=20`를 `OwnerProductRepository.fetchProducts` → `ProductListViewModel` → `ProductListScreen`으로 연결한다. 기존 생성 서비스와 Hilt 바인딩을 재사용한다.
...
- 등록 상품 탭은 `ALL`, `ON_SALE`, `RUNNING_LOW`, `SOLD_OUT`, `CLOSED` 필터를 각각 전체·판매중·품절 임박·품절·판매 종료로 표시한다. 필터를 바꾸면 해당 값을 서버에 보내고 0페이지부터 다시 조회하며 선택 상태를 `SavedStateHandle`에 보존한다.
- 등록 상품 탭의 전체 건수와 현재 필터 결과 건수는 로드한 항목 수가 아니라 서버의 `totalElements`를 사용한다. 찜 현황 필터 상태와 등록 상품 필터 상태는 서로 독립적으로 유지한다.

## 서버 계약 대기

- 2026-09-19-v2 상품 상세·미리보기 응답의 `ingredientTags`는 정수 ID 집합이다. 식자재 검색·추천 API의 응답 객체와 혼동하거나 태그 이름을 화면에서 임의로 생성하지 않는다.
- 생성 기준은 `mangro-app-openapi-2026-09-19-v2-merged.json`과 일치하는 체크섬을 사용한다. 배포 서버와의 실시간 동일성은 확인하지 못했다.
...
- 등록 상품/찜 현황 탭을 제공한다. 등록 상품 탭은 서버의 상품 상태 필터를 사용하고 찜 현황 탭만 전체·픽업완료·픽업불가·찜 취소·만료된 찜 필터를 사용한다.
