...
- 사진: 최종 확정 정책에 따라 최대 5장. 태그는 `1155:14029`의 최대 5개를 적용한다.
- 품목명: 목록 예시의 25자 내외 문구를 근거로 최대 25 Unicode 코드포인트 적용. 공백만 입력할 수 없으며 앞뒤 공백은 제거한다. 정확한 정책 확정 필요.
- 태그는 자유 입력이며 첫 태그를 대표로 표시한다. 미리보기 분류는 매장 조회 응답의 카테고리를 표시한다.
...
단위 테스트는 가격·수량 경계값, 할인율 오버플로, Unicode 품목명, 사진/태그 제한, 찜 수량 대비 가용 재고를 확인한다. 앱의 `ownerAndroidTest`는 호이스팅된 데이터로 등록/재고 UI를 검사한다. 실행 결과는 작업 완료 보고를 참조한다.
...
최종 저장 시 매장 승인 상태를 재확인한 뒤 `:data:owner:product`가 첫 사진만 multipart `file`로 업로드하고 반환 URL로 상품을 등록한다. 사진 배열이 비어 있으면 업로드를 생략하고 필수 문자열 `photoUrl`은 빈 문자열로 전송한다. 기존 사진 선택 UI의 최대 5장과 로컬 미리보기는 유지하며, 최종 저장된 모델에는 서버가 반환한 사진 한 장만 반영한다.
