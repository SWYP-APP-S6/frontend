...
| `:remote:owner` | Android library 모듈 | Owner 17 API 생성 (v3 식자재 검색·추천 및 상품 목록 포함) |
...
- 2026-09-18 API 갱신: `GET /owner/products` 상품 목록과 페이지 응답 모델을 기존 v3 명세에 병합했다. 기존 식자재 검색·추천과 객체형 `ingredientTags` 계약은 유지한다.
